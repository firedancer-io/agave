    ...
    while chunk_start != txns.len() {
        ...
        for i in chunk_start..chunk_end {
            ts_load_end[i] = start_time_ns
                           + (metrics.execute_timings.metrics.index(ExecuteTimingType::CheckUs).0 * 1000u64)
                           + (metrics.execute_timings.metrics.index(ExecuteTimingType::ValidateFeesUs).0 * 1000u64)
                           + (metrics.execute_timings.metrics.index(ExecuteTimingType::FilterExecutableUs).0 * 1000u64)
                           + (metrics.execute_timings.metrics.index(ExecuteTimingType::ProgramCacheUs).0 * 1000u64)
                           + (metrics.execute_timings.metrics.index(ExecuteTimingType::LoadUs).0 * 1000u64);
            ts_exec_end[i] = ts_load_end[i] + (metrics.execute_timings.metrics.index(ExecuteTimingType::ExecuteUs).0 * 1000u64);
        }
