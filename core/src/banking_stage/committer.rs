...
pub extern "C" fn fd_ext_bank_load_and_execute_txns( bank: *const std::ffi::c_void, txns: *const std::ffi::c_void, txn_count: u64, out_load_results: *mut i32, out_executing_results: *mut i32, out_executed_results: *mut i32, out_consumed_cus: *mut u32 ) -> *mut std::ffi::c_void {
    ...
    for i in 0..txn_count {
        if let Some(processed_tx) = output.processing_results[i as usize].processed_transaction() {
            ...
            if let Some(executed_tx) = processed_tx.execution_details() {
                unsafe { *out_executing_results.offset(i as isize) = 0 };
                unsafe { *out_consumed_cus.offset(i as isize) = executed_tx.executed_units.try_into().unwrap() };
                match &executed_tx.status {
                ...
                unsafe { *out_executing_results.offset(i as isize) = transaction_error_to_code( &output.processing_results[i as usize].flattened_result().err().unwrap() ) };
            }
        } else {
            unsafe { *out_executing_results.offset(i as isize) = transaction_error_to_code( &output.processing_results[i as usize].flattened_result().err().unwrap() ) };
            unsafe { *out_load_results.offset(i as isize) = transaction_error_to_code( &output.processing_results[i as usize].flattened_result().err().unwrap() ) };
        }
    }
