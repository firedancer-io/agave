...
use {
    ...
    solana_sdk::{
        clock::{BankId, Slot, MAX_PROCESSING_AGE, NUM_CONSECUTIVE_LEADER_SLOTS}, hash::Hash, pubkey::Pubkey, saturating_add_assign, signature::{Keypair, Signature, Signer}, timing::timestamp, transaction::Transaction
    },
    ...
    fn process_replay_results(
        ...
        for replay_result in replay_result_vec {
            ...
            if bank.is_complete() {
                ...
                memory[64..72].copy_from_slice(&tips.to_le_bytes());
                ...
                unsafe {
                    fd_ext_plugin_publish_replay_stage(2, memory.as_ptr(), 72);
                }
