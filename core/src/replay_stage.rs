...
impl ReplayStage {
        ...
        let run_replay = move || {
            ...
            loop {
                ...
                for slot in newly_computed_slot_stats {
                    ...
                    let (confirmed_slots, mostly_confirmed_forks) = Self::confirm_forks(
                        &tower,
                ...
                if let Some((ref vote_bank, ref switch_fork_decision)) = vote_bank {
                    ...
                    if let Some(threshold_ahead_count) = tower.get_threshold_ahead_count() {
                            ...
                            ReplayStage::first_mostly_confirmed_bank(vote_bank.clone(), &progress)
                        {
                            ...
                            // Now pop votes from the tower's vote_state only if they are on a different fork than the
                            // votes.  This is only allowed with VoteStateUpdate, which no longer requires
                            // expirations.
    ...
    fn handle_votable_banks(
        ...
        duplicate_slots_tracker: &mut DuplicateSlotsTracker,
        gossip_duplicate_confirmed_slots: &mut DuplicateConfirmedSlots,
        unfrozen_gossip_verified_vote_hashes: &mut UnfrozenGossipVerifiedVoteHashes,
