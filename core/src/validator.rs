...
impl VSPRwLock {
    pub fn write(&self) -> std::sync::LockResult<VSPRwLockWriteGuard<'_>> {
        Ok(VSPRwLockWriteGuard { inner: self.inner.write().unwrap() })
    }

    pub fn read(&self) -> std::sync::LockResult<std::sync::RwLockReadGuard<'_, ValidatorStartProgress>> {
        self.inner.read()
    }
}
...
impl Validator {
    ...
    pub fn new_with_exit(
        ...
        *start_progress.write().unwrap() = ValidatorStartProgress::StartingServices;
        ...
        const FIREDANCER_CLUSTER_NODE_CNT: u64 = 108_000 - 1; /* -1 because it doesn't include itself */
        unsafe extern "C" {
        ...
        fn firedancer_publish_gossip_peers(cluster_info: &Arc<ClusterInfo>) {
            ...
            for (i, node) in all_peers.iter().enumerate().take(len) {
                ...
                if let Some(version) = version {
                    ...
                    let client_id = u16::try_from(version.client().clone()).unwrap_or(u16::MAX);
                    memory[offset+58..offset+60].copy_from_slice(&client_id.to_le_bytes());
                } else {
                    memory[offset+42..offset+60].fill(0);
                }
            ...
            unsafe {
                fd_ext_plugin_publish_periodic(4, memory.as_ptr(), 8 + len as u64 * (60 + 12 * 6));
            }
        }
        ...
        fn firedancer_publish_vote_accounts(bank_forks: &Arc<RwLock<BankForks>>, block_commitment_cache: &Arc<RwLock<BlockCommitmentCache>>) {
            ...
            unsafe {
                fd_ext_plugin_publish_periodic(5, memory.as_ptr(), 8 + len as u64 * 112);
            }
        }
        ...
        fn firedancer_publish_validator_info(account_indexes: &solana_accounts_db::accounts_index::AccountSecondaryIndexes, bank_forks: &Arc<RwLock<BankForks>>, block_commitment_cache: &Arc<RwLock<BlockCommitmentCache>>, force_accounts_scan: bool) {
            ...
            use solana_accounts_db::accounts_scan::ScanConfig;
            ...
            let mut account_publish = |account: &AccountSharedData, lsched: &Arc<HashMap<Pubkey, NodeVoteAccounts>>| -> Option<()> {
                ...
                unsafe {
                    fd_ext_plugin_publish_validator_info(7, memory.as_ptr(), 608);
                }
            ...
            let res = if acct_index_inactive {
                bank.accounts().accounts_db.scan_accounts_unguarded(&bank.ancestors, &config::id(), |_, account, _| { account_publish(&account, lsched); }, &ScanConfig::default());
                Some(())
            } else {
                bank.accounts().accounts_db.index_scan_accounts(&bank.ancestors, bank.bank_id(), IndexKey::ProgramId(config::id()), scan_fn, &ScanConfig::default()).ok().map(|_| ())
            };

            if res.is_none() {
                warn!("Failed to perform config accounts scan");
            }
        }
        ...
        {
            let account_indexes = config.rpc_config.account_indexes.clone();
...
unsafe extern "C" {
    ...
    fn fd_ext_store_initialize( store: *const std::ffi::c_void, pinnable_slice: *const std::ffi::c_void, write_batch: *const std::ffi::c_void );
}
...
> {
    ...
    *start_progress.write().unwrap() = ValidatorStartProgress::LoadingLedger;
    ...
    // FIREDANCER: Allocate the long-lived memory the blockstore needs and notify Firedancer of the blockstore.
    unsafe {
        fd_ext_store_initialize(
            Arc::into_raw(Arc::clone(&blockstore)) as *const std::ffi::c_void,
            Box::into_raw(Box::new(blockstore.new_pinnable_slice())) as *const std::ffi::c_void,
            Box::into_raw(Box::new(blockstore.get_write_batch().unwrap()))
