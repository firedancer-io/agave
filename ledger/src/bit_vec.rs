...
impl<const NUM_BITS: usize> BitVec<NUM_BITS> {
    ...
    /// Highest set bit strictly below `bound` (u64-chunked scan; equivalent to `range(..bound).iter_ones().next_back()`).
    ///
    /// # Examples
    ///
    /// ```
    ...
    pub fn prev_set_bit(&self, bound: usize) -> Option<usize> {
        ...
        let mut chunks = self.words[..last_word_idx].rchunks_exact(8);
        ...
        for chunk in &mut chunks {
            ...
            if chunk_word != 0 {
                ...
                return Some((chunk_end_word_idx - 8) * BITS_PER_WORD + msb);
            }
            chunk_end_word_idx -= 8;
        }
    ...
    /// Lowest set bit at or above `from` (u64-chunked scan; equivalent to `range(from..).iter_ones().next()`).
    ///
    /// # Examples
    ///
    /// ```
    ...
    pub fn next_set_bit(&self, from: usize) -> Option<usize> {
        ...
        // A hit can exceed NUM_BITS only via unmasked tail bits in the final word (e.g. from
        // deserialized data); treat those as unset, like the masking iterator path does.
...
mod tests {
    ...
    fn test_prev_next_set_bit_edges() {
        ...
        // Single bit at word, u64-chunk, and array boundaries.
    ...
    proptest! {
        ...
        // Property: prev_set_bit/next_set_bit match their iterator equivalents,
        // across empty sets, chunk boundaries, and the NUM_BITS edge.
        #[test]
