...
pub extern "C" fn fd_ext_blockstore_insert_shreds(
    ...
    stride: u64,
    is_trusted: i32,
    pinnable_slice: *mut std::ffi::c_void,
    write_batch: *mut std::ffi::c_void,
) {
    ...
    let pinnable_slice = unsafe { &mut *(pinnable_slice as *mut DBPinnableSlice) };
    let write_batch = unsafe { &mut *(write_batch as *mut WriteBatch) };
