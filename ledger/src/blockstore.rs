...
pub extern "C" fn fd_ext_blockstore_insert_shreds(blockstore: *const std::ffi::c_void, shred_cnt: u64, shred_bytes: *const u8, shred_sz: u64, stride: u64, is_trusted: i32) {
    ...
    let InsertResults {
        ...
        duplicate_shreds,
    } = blockstore.do_insert_shreds(shreds, None, is_trusted!=0, None, &mut BlockstoreInsertionMetrics::default()).unwrap();

    for shred in duplicate_shreds {
        /* In normal Agave code, these shreds are sent to the window
           service which handles them in run_check_duplicate.
           Frankendancer support for duplicate shreds is a bit different.
           We don't send duplicate proofs over gossip (but we do receive them).
           The following code is effectively inlined from the
           check_duplicate lambda in window_service.rs, with the error
           handling inlined from the caller.  We also omit sending anything to consensus via duplicate_slots_sender.  This means if we've already replayed the block, we'll ignore equivocating shreds that arrive late. */
        ...
        match shred {
            ...
            PossibleDuplicateShred::Exists(shred) => {
                ...
                if !blockstore.has_duplicate_shreds_in_slot(shred_slot) { // A duplicate is not already recorded
                    if let Some(existing_shred_payload) = blockstore.is_shred_duplicate(&shred) { // actually a duplicate
...
pub extern "C" fn fd_ext_blockstore_create_block0(ledger_path: *const i8, shred_cnt: u64, shred_bytes: *const u8, shred_sz: u64, stride: u64) {
    ...
    let blockstore = Blockstore::open_with_options(
        ledger_path,
        BlockstoreOptions {
            access_type: crate::blockstore_options::AccessType::Primary,
            recovery_mode: None,
