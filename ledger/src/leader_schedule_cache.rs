...
impl LeaderScheduleCache {
    ...
    unsafe fn firedancer_send_leader_schedule(
        ...
        if let Some(lock) = send_firedancer.as_ref() {
            ...
            let use_vote_keyed_leader_schedule = match bank.should_use_vote_keyed_leader_schedule(epoch) {
                Some(true) => 1u64,
                _ => 0u64,
            };
            ...
            fd_ext_poh_publish_leader_schedule(memory.as_ptr(), (FIREDANCER_PACKET_HEADER_SZ + stakes.len() * FIREDANCER_PACKET_RECORD_SZ) as u64);
        }
    }
