...
pub struct PohRecorder {
  ...
  pub shared_working_bank: SharedWorkingBank,
}
...
impl PohRecorder {
    ...
    pub fn new_with_clear_signal(
        ...
        (Self { is_exited, shared_working_bank: SharedWorkingBank::empty() }, dummy1.1)
    }
    ...
    pub fn set_bank(&mut self, bank_with_scheduler: BankWithScheduler) {
        ...
        self.shared_working_bank.0.store(Some(bank.clone()));
    ...
    pub fn reset(&mut self, reset_bank: Arc<Bank>, _next_leader_slot: Option<(Slot, Slot)>) {
        /* Must be implemented, used by replay stage. */
        self.shared_working_bank.0.store(None);
    ...
    pub fn shared_working_bank(&self) -> SharedWorkingBank {
        /* Must be implemented. Called by banking stage, and later used in shared_working_bank.load() */
        self.shared_working_bank.clone()
    }

    pub fn shared_tick_height(&self) -> SharedTickHeight {
        /* Return dummy SharedTickHeight, as it's only used in tests. */
        SharedTickHeight::new(0)
    }

    pub fn shared_leader_first_tick_height(&self) -> SharedLeaderFirstTickHeight {
        /* Return dummy SharedLeaderFirstTickHeight, as it's only used in tests. */
        SharedLeaderFirstTickHeight::new(None)
    }
