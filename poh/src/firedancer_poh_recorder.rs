...
extern "C" {
    ...
    fn fd_ext_poh_reset(reset_bank_slot: u64, reset_blockhash: *const u8, hashcnt_per_tick: u64, features_activation_slot: *const u64);
...
impl PohRecorder {
    ...
    pub fn reset(&mut self, reset_bank: Arc<Bank>, _next_leader_slot: Option<(Slot, Slot)>) {
        ...
        unsafe { fd_ext_poh_reset(  reset_bank_slot,reset_bank_blockhash.as_ref().as_ptr(),
                                    hashes_per_tick, features_activation_slot.as_ref().as_ptr() ) };
    }
}
