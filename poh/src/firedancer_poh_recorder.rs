...
use solana_cost_model::block_cost_limits::{simd_0207_block_limits, MAX_WRITABLE_ACCOUNT_UNITS, MAX_BLOCK_UNITS, MAX_VOTE_UNITS};
...
impl PohRecorder {
    ...
    pub fn set_bank(&mut self, bank_with_scheduler: BankWithScheduler, _track_transaction_indexes: bool) {
        ...
        let (mut cus_account_cost_limit, mut cus_block_limit, mut cus_vote_cost_limit) = (MAX_WRITABLE_ACCOUNT_UNITS, MAX_BLOCK_UNITS, MAX_VOTE_UNITS);
        if bank
           .feature_set
           .is_active(&feature_set::raise_block_limits_to_50m::id())
        {
            (cus_account_cost_limit, cus_block_limit, cus_vote_cost_limit) = simd_0207_block_limits();
        }
    ...
    pub fn reset(&mut self, reset_bank: Arc<Bank>, _next_leader_slot: Option<(Slot, Slot)>) {
        ...
        let block_id_ptr = if let Some(block_id) = reset_bank.block_id() {
