...
use crate::old_poh_recorder::{SharedLeaderState, LeaderState};
...
static FIREDANCER_SHARED_LEADER_STATE: std::sync::atomic::AtomicU64 = std::sync::atomic::AtomicU64::new(0);
...
pub extern "C" fn fd_ext_poh_register_tick( bank: *const c_void, hash: *const u8 ) {
    ...
    let bank_ref = unsafe { &*(bank as *const Bank) };
    bank_ref.register_tick(&hash, &BankWithScheduler::no_scheduler_available());

    let shared_leader_state_ptr = FIREDANCER_SHARED_LEADER_STATE.load(Ordering::Acquire);
    if shared_leader_state_ptr != 0 {
        let shared_leader_state: &SharedLeaderState = unsafe { &*(shared_leader_state_ptr as *const SharedLeaderState) };
        shared_leader_state.increment_tick_height();
    }
}
...
impl PohRecorder {
    ...
    fn register_shared_leader_state(&self) {
        use std::sync::atomic::Ordering;
        FIREDANCER_SHARED_LEADER_STATE.store(&self.shared_leader_state as *const SharedLeaderState as u64, Ordering::Release);
    }
    ...
    pub fn set_bank(&mut self, bank_with_scheduler: BankWithScheduler) {
        ...
        self.register_shared_leader_state();
