...
impl PartialEq for Bank {
    fn eq(&self, other: &Self) -> bool {
        ...
        let Self {
            ...
            blockhash_queue,
            ancestors,
            // FIREDANCER: track slot parents for blockhash expiry
            slot_parents,
            hash,
            parent_hash,
            parent_slot,
...
pub struct Bank {
    ...
    /// FIREDANCER: store the slot number of 151 parents of this slot, and this
    /// bank's slot number.
    /// In the typical (no-skip) case, if the current slot is s, a transaction
    /// with a recent blockhash (not durable nonce) value of the blockhash of
    /// slot b is allowed iff b is in [s-151, s).  There are 151 such values,
    /// presumably due to an initial off-by-one error that has become part of
    /// the protocol.  Note that b cannot be >=s, since it can't reference a
    /// blockhash that doesn't exist yet, and the blockhash cannot be computed
    /// until the block is complete.  For convenience, this deque also stores s,
    /// which is why it has capacity 152.
...
impl Bank {
    fn default_with_accounts(accounts: Accounts) -> Self {
        let mut bank = Self {
            ...
            // FIREDANCER: track slot parents for blockhash expiry.  See above
            // for capacity.
    ...
    fn _new_from_parent(
        ...
        // FIREDANCER: Update slot parents with this new slot.  Remove any slot
        // nubmers that are too old.
    ...
    pub(crate) fn new_from_fields(
        ...
        // FIREDANCER: track slot parents for blockhash expiry.  See above for
        // capacity.
