...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn fd_ext_bank_load_account( bank: *const std::ffi::c_void, with_fixed_root: i32, addr: *const u8, out_owner: *mut u8, out_data: *mut u8, out_data_sz: *mut u64 ) -> i32 {
...
pub static MAINNET_TIP_ACCOUNTS: LazyLock<HashSet<Pubkey>> = LazyLock::new(|| MAINNET_TIP_ACCOUNTS1.iter().cloned().collect());
pub static TESTNET_TIP_ACCOUNTS: LazyLock<HashSet<Pubkey>> = LazyLock::new(|| TESTNET_TIP_ACCOUNTS1.iter().cloned().collect());
pub static EMPTY_TIP_ACCOUNTS: LazyLock<HashSet<Pubkey>> = LazyLock::new(HashSet::new);
...
impl Bank {
    ...
    fn apply_cost_tracker_limits_for_active_features(&mut self) {
        ...
        // slot_params here (discarding any default() value), so the override must be applied here.
        unsafe extern "C" {
