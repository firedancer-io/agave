    ...
    loop {
        ...
        if bank.slot == 32 {
            ...
            assert_eq!(
                bank.hash().to_string(),
                if deprecate_rent_exemption_threshold {
                    "BEwYsox6ArHzahDAvApyuLfmCrrEqYRK6X1vUBgiCKLk"
                } else {
                    "Bd3HGiUuGSSe6HL6L2SVRgV1kYDfnLCKUQwySds6HHjN"
                },
            );
        }
        if bank.slot == 128 {
            ...
            assert_eq!(
                bank.hash().to_string(),
                if deprecate_rent_exemption_threshold {
                    "6BEVxtfX5kudKQyPCUk41vLtkhbX8ZXGPa2BS5A1SJxN"
                } else {
                    "8GyPiXmEA7LEfpjYSjA7RXY96dXkueAJkY9ZQApUy8Ej"
                },
            );
            break;
        }
...
fn test_bank_hash_deterministic_with_stakes_cache() {
    ...
    assert_eq!(
        bank2.hash().as_bytes(),
        &[
            39, 34, 123, 219, 250, 198, 218, 222, 15, 184, 165, 5, 177, 150, 99, 169, 179, 59, 145,
            212, 155, 243, 105, 143, 195, 55, 129, 38, 31, 188, 170, 152
        ]
    );
}
