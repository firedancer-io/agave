...
impl StakesCache {
    ...
    pub(crate) fn check_and_store(
        ...
        can_safely_remove_inactive_stakes: bool,
    ) {
...
impl Stakes<StakeAccount> {
    ...
    pub(crate) fn activate_epoch(
        ...
        // These delegations contribute no stake to `delegated_stakes` nor to
        // `vote_accounts`, so no stake bookkeeping is needed here.
    ...
    fn upsert_stake_delegation(
        ...
        if can_safely_remove_inactive_stakes
            && activation_status.effective == 0
            && activation_status.activating == 0
        {
