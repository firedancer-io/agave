...
#[unsafe(no_mangle)]
pub extern "C" fn fd_ext_admin_rpc_set_identity(identity_keypair: *const u8, require_tower: i32, require_vote_history: i32) -> i32 {
    loop {
    ...
    match AdminRpcImpl::set_identity_keypair(metadata.clone(), identity_keypair, require_tower!=0, require_vote_history!=0) {
        Ok(()) => 0,
        Err(err) => {
