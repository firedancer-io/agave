...
pub extern "C" fn fd_ext_admin_rpc_set_identity(identity_keypair: *const u8, require_tower: i32) -> i32 {
    ...
    let identity_keypair = Keypair::try_from(identity_keypair).unwrap();

    // FIREDANCER: require_vote_history is hardcoded to agave's default (true). It is
    // inert until Alpenglow is enabled. The C side changes to support
    // `--do-not-require-vote-history` will be made in a separate PR.
    match AdminRpcImpl::set_identity_keypair(metadata.clone(), identity_keypair, require_tower!=0, true) {
        Ok(()) => 0,
        Err(err) => {
